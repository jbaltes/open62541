 ...
 *    "The target is a MAC address, an IP address or a registered name
 *     like a hostname. The format of a MAC address is six groups of
 *     hexadecimal digits, separated by hyphens (e.g. 01-23-45-67-89-ab).
 *     A system may also accept hostnames and/or IP addresses
 *     if it provides means to resolve it to a MAC address (e.g. DNS and
 *     Reverse-ARP)."
 *
...
UA_parseHardwareAddress(UA_String* target, UA_Byte* destinationMac) {
    ...
    for(idx = 0; idx < ETH_ALEN; idx++) {
        size_t progress;
        progress = UA_readNumberWithBase(&target->data[curr],
...
UA_PubSubChannelEthernet_open(const UA_PubSubConnectionConfig *connectionConfig) {
    ...
    if (UA_parseHardwareAddress(&target, channelDataEthernet->targetAddress) != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "PubSub Connection creation failed. Invalid destination MAC address.");
        return NULL;
    }
    ...
    int sockFd;
    if ((sockFd = UA_socket(PF_PACKET, SOCK_RAW, 0)) < 0) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
...
UA_PubSubChannelEthernet_regist(UA_PubSubChannel *channel, UA_ExtensionObject *transportSettings) {
    ...
    channelDataEthernet = (UA_PubSubChannelDataEthernet *) channel->handle;
...
UA_PubSubChannelEthernet_receive(UA_PubSubChannel *channel, UA_ByteString *message, UA_ExtensionObject *transportSettings, UA_UInt32 timeout) {
    ...
    dataLen = recvmsg(channel->sockfd, &msg, 0);
    if (dataLen < 0) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
            "PubSub connection receive failed. Receive message failed.");
        return UA_STATUSCODE_BADINTERNALERROR;
    } else if ((size_t)dataLen < sizeof(eth_hdr)) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
...
UA_PubSubTransportLayerEthernet() {
    ...
    pubSubTransportLayer.transportProfileUri = UA_STRING("http://opcfoundation.org/UA-Profile/Transport/pubsub-eth-uadp");
